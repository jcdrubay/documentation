...
Generate EPCs for tracked products directly in Odoo. Ideally, products should be tracked by serial
number, but this is not a requirement. A GTIN-14-compatible barcode must be set on the product.
...
#. Click the :guilabel:`Moves` smart button. Select all received items, then click the :icon:`fa-cog`
   :guilabel:`Actions` button, then select :guilabel:`Export`.
