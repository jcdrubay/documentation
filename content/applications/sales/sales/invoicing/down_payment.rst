...
payment (percentage)`, and type `50` in the :guilabel:`Down Payment Amount` field.
...
When the customer wants to pay the remaining amount of the order, another invoice must be created.
...
window that appears, there are two new fields: :guilabel:`Already invoiced` and :guilabel:`Amount to
invoice`.
...
Both down payments are present in the :guilabel:`Invoice Lines` tab.
...
After clicking :guilabel:`Create Payment`, Odoo reveals the customer invoice, now with a green
:guilabel:`In Payment` banner in the upper-right corner. The process is now complete, and the 100%
down payment has been successfully applied.
...
.. image:: down_payment/income-account.png
   :alt: The product form for :guilabel:`Acoustic Bloc Screens` with the Income Account field
   expanded and multiple options visible.
